...
import { getAllSilos, findSiloByNumber, regenerateAllSiloData, fetchLiveSiloData } from '../services/siloData';
...
export const useSiloSystem = () => {
  ...
  // Get auto test state from global context
  const { autoTestState, updateAutoTestState } = useSystemData();
  ...
  // Derived auto test state from global context
  const {
    isRunning: isAutoTestRunning,
    currentIndex: autoTestCurrentIndex,
    progress: autoReadProgress,
    completed: autoReadCompleted
  } = autoTestState;
    ...
    const countdownInterval = setInterval(() => {
      setWaitTimeRemaining(prev => {
        if (prev <= 1) {
          ...
          updateAutoTestState({ completed: false, progress: 0 });
  ...
  const startAutoRead = useCallback(() => {
    if (readingMode === 'auto' && autoReadInterval.current) {
      ...
      setIsReading(false);
      setReadingSilo(null);
      updateAutoTestState({ progress: 0 });
      setReadingMode('none');
      updateAutoTestState({ completed: false });
      return;
    }

    // Fetch live data before starting auto readings
    fetchLiveSiloData()
      .then(() => {
        setReadingMode('auto');
        setIsReading(true);
        ...
        // Process first silo immediately
        if (allSilos.length > 0) {
          ...
          updateAutoTestState({ progress: (1 / allSilos.length) * 100 });
          currentIndex = 1; // Start from the second silo next time
        }
        ...
        const interval = setInterval(() => {
          ...
          if (timeoutCounter > maxTimeout) {
            ...
            setIsReading(false);
            setReadingSilo(null);
            updateAutoTestState({ progress: 100 });
            setReadingMode('none');
            updateAutoTestState({ completed: true });
            return;
          }
          ...
          updateAutoTestState({ progress: ((currentIndex + 1) / allSilos.length) * 100 });
      ...
      .catch((error) => {
        console.error('Error fetching live data for auto readings:', error);
        setIsReading(false);
        setReadingSilo(null);
        updateAutoTestState({ progress: 0 });
        setReadingMode('none');
  ...
  }, [readingMode, isReading, autoReadCompleted, getSiloTestDuration, startAutoRestartWait]);
  ...
  return {
    ...
    isReading,
    readingSilo,
    autoReadProgress: autoReadProgress,
    autoReadCompleted: autoReadCompleted,
    dataVersion,
    manualTestDuration,
    autoTestInterval,
    isWaitingForRestart,
